...
void *gc_link_small(void *ptr, uint16_t object_size) {
  ...
  for (int i = 0; i < (CHUNKSIZE / object_size) - 1; i++) {
    *((void **)current) = (void *)(current + object_size);
    current = (void *)((uint8_t *)current + object_size);
  }
  *((void **)current) = NULL;
...
void gc_debug_small(void *ptr) {
  ...
  int w = (((unsigned long)ptr) & 0xFFF) / MAX_BITMAP_SIZE;
  printf("pointer -> %p, word -> %u\n", ptr, metadata->bitmap[w]);
}
